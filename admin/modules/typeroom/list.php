...
$listTyperoom = getRaw("SELECT * FROM typeroom ORDER BY type_id LIMIT $offset, $rowsPerPage");
$countResult = getRaw("SELECT COUNT(*) AS total FROM typeroom");
        ...
        require_once('templaces/layout/navbar.php');?>
            <div class="content" id="content">
                <div class="container-fluid">
                    ...
                    <p><a href="?modules=typeroom&action=add" class=" btn btn-success btn-sm">Thêm loại phòng <i
            ...
            if(!empty($listTyperoom)):
                foreach ($listTyperoom as $index => $item):
            ...
            ?>
                            <tr>
                                ...
                                <td><a href="<?php echo _WEB_HOST;?>?modules=typeroom&action=edit&id=<?php echo $item['type_id'];?>"
                                        class=" btn btn-warning btn-sm"><i class="fas fa-pen"></i></a></td>
                                <td><a onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"
                                        href="<?php echo _WEB_HOST;?>?modules=typeroom&action=delete&id=<?php echo $item['type_id'];?>"
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <li class="page-item"><a class="page-link"
                                    href="?modules=typeroom&action=list&page=<?php echo ($page - 1); ?>">Previous</a>
                            </li>
                            <?php endif; ?>
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <li class="page-item <?php if ($page === $i) echo 'active'; ?>"><a class="page-link"
                                    href="?modules=typeroom&action=list&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                            </li>
                            <?php endfor; ?>
                            <?php if ($page < $totalPages): ?>
                            <li class="page-item"><a class="page-link"
                                    href="?modules=typeroom&action=list&page=<?php echo ($page + 1); ?>">Next</a></li>
                            <?php endif; ?>
                        </ul>
                    </nav>
                </div>
