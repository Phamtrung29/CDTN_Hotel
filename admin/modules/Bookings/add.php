...
if (isPost()) {
    ...
    if (empty($error)) {
        ...
        $insertStatus = insert('bookings', $dataInsert);
        if ($insertStatus) {
            setFlashData('smg', 'Thêm đặt phòng thành công!');
            setFlashData('smg_type', 'success');
            redirect('?page=Bookings/list');
        } else {
            setFlashData('smg', 'Thêm đặt phòng không thành công!');
            setFlashData('smg_type', 'danger');
            redirect('?modules=Bookings&action=add');
        }
    } else {
        ...
        require_once('templaces/layout/navbar.php');?>
            ...
            <div class="row">
                <div class="col-4 mg-home" style="margin: 50px auto;">
                    <h2 class="text-center text-uppercase">Thêm đặt phòng</h2>
                    <?php
        if (!empty($smg)) {
