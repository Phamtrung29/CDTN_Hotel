...
$listRooms = getRaw("SELECT * FROM rooms ORDER BY room_id LIMIT $offset, $rowsPerPage");
$countResult = getRaw("SELECT COUNT(*) AS total FROM rooms");
        ...
        require_once('templaces/layout/navbar.php');?>
            <div class="content" id="content">
                <div class="container-fluid">
                    <h2>Quản lý phòng</h2>
                    <p><a href="?modules=room&action=add" class="btn btn-success btn-sm">Thêm Phòng <i
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php
            if(!empty($listRooms)):
                    foreach ($listRooms as $index => $room):
            ...
            ?>
                            <tr>
                                ...
                                <td>
                                    <div class="room-image">
                                        <img class="img-fluid"
                                            src="<?php echo _WEB_HOST_TEMPLACES ?>/<?php echo $room['image_url']; ?>"
                    ...
                    ?>
                                ...
                                <td><a href="<?php echo _WEB_HOST;?>?modules=room&action=edit&id=<?php echo $room['room_id'];?>"
                                        class=" btn btn-warning btn-sm"><i class="fas fa-pen"></i></a></td>
                                <td><a onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"
                                        href="<?php echo _WEB_HOST;?>?modules=room&action=delete&id=<?php echo $room['room_id'];?>"
            ...
            ?>
                            <tr>
                                <td colspan="10">
                                    <div class="alert alert-danger text-center">Chưa có phòng nào</div>
                                </td>
                            </tr>
                            <?php
            endif;
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <li class="page-item"><a class="page-link"
                                    href="?modules=room&action=list&page=<?php echo ($page - 1); ?>">Previous</a></li>
                            <?php endif; ?>
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <li class="page-item <?php if ($page === $i) echo 'active'; ?>"><a class="page-link"
                                    href="?modules=room&action=list&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php endfor; ?>
                            <?php if ($page < $totalPages): ?>
                            <li class="page-item"><a class="page-link"
                                    href="?modules=room&action=list&page=<?php echo ($page + 1); ?>">Next</a></li>
                            <?php endif; ?>
                        </ul>
                    </nav>
